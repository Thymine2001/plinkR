...
#' @param read_outputs What to read: "auto" (infer from --recode/--make-bed),
#'   "none" (skip reading), "binary" (BED/BIM/FAM), "text" (PED/MAP), or "all"
#' @param use_fread Logical; use data.table::fread for faster reading (default TRUE)
#' @param exe Character; explicit path to PLINK executable. If NULL, searches PATH
...
#'
#' @export
plink <- function(cmd, read_outputs = c("auto", "none", "binary", "text", "all"), 
  ...
  # Determine PLINK executable path
  if (!is.null(exe)) {
    # User provided explicit exe path
    if (!file.exists(exe)) {
      stop("Specified exe '", exe, "' does not exist")
    }
  } else if (!is.null(plink_dir)) {
    # User provided plink_dir, search for plink2/plink
    ...
    # Try stored option from .onLoad, or search PATH
    ...
    if (exe == "") {
      stop(
        ...
        "  1. Add PLINK to your PATH environment variable, or\n",
        "  2. Pass plink_dir = '/path/to/plink/dir', or\n",
        "  3. Pass exe = '/full/path/to/plink'"
      )
    }
  }
  ...
  out_geno <- if (!is.null(out_prefix) && read_outputs != "none") read_plink(out_prefix, format = out_format, use_fread = use_fread) else list(bed = NULL, fam = NULL, bim = NULL, ped = NULL, map = NULL)

  # Collect reports based on args and discovered outputs (no need for --out)
  report <- if (!is.null(out_prefix)) .collect_reports(out_prefix, args, use_fread = use_fread,
                                                      start_time = start_time, end_time = end_time) else list()
